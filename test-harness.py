...
# 2. huff-compress.py times out -- time limit set to 10 seconds
...
try:
    cp = run(["./huff-compress.py","-s",symbolmodel,args.infile],check=True,timeout=timeout)
...
# 2. huff-decompress.py times out -- time limit set to 10 seconds
...
try:
    cp = run(["./huff-decompress.py",root +'.bin'],check=True,timeout=timeout)
